...
/**
 * 这是关于FastJson的一个使用Demo，在Java环境下验证的； 使用的时候可以参照这个例子进行回忆！
 * 备注：下面写的Json格式的数据"前面的\，意思是告诉编译器忽略掉它后面的"。 后面定义有三个类：User、Group、Weibo
 * 
 * @author liuzhao
 */
public class JsonDemo {
	...
	private static void bean2JsonM5() {
		...
		String jsString = JSONArray.toJSONString(weibo);
		System.out.println("JSON.toJSONString"+string);
		System.out.println("JSONArray.toJSONString"+jsString);
		/**
